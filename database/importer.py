...
def _invia(blocco: list) -> int:
    payload = {"connettore": settings.connettore, "offerte": blocco}
    ...
    if risposta.status_code == 401:
        raise SystemExit("Token non valido o scaduto (401): rigenera il JWT nel workflow.")
    if risposta.status_code == 400:
        print(f"Payload rifiutato (400): {risposta.text}", file=sys.stderr)
        return 0
    if risposta.status_code >= 400:
        print(f"Errore {risposta.status_code}: {risposta.text}", file=sys.stderr)
        return 0
...
def import_offers(offerte: list) -> int:
    ...
    dimensione = min(settings.batch_size, MAX_PER_RICHIESTA)
    totale = 0
    for blocco in _blocchi(offerte, dimensione):
        importate = _invia(blocco)
        print(f"Inviate {len(blocco)} offerte -> importate {importate}")
        totale += importate
    return totale
