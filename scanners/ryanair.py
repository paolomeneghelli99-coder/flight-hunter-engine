    ...
    def scan(self) -> list:
        ...
        for origine in self.origini:
            ...
            for voce in dati.get("fares", []):
                ...
                citta = (arrivo.get("city") or {}).get("name") or arrivo.get("name")
                ...
                offerte.append(Offerta(
                    ...
                    prezzo=float(prezzo),
                    valuta=(out.get("price") or {}).get("currencyCode", settings.valuta),
                    data_partenza=partenza,
                    data_ritorno=None,
                    link_prenotazione=(
                        f"{BOOKING_URL}?adults=1&dateOut={partenza}"
                        f"&originIata={origine}&destinationIata={arrivo.get('iataCode', '')}"
                    ),
